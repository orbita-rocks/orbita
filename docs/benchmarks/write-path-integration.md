...
This change takes five optimizations from the September 14 write-path spike:
...
- Peer replication overlaps local append/fsync. The committed prefix remains
  capped by local durability, and quiesce waits for the local flusher before
  truncating its uncommitted tail.
...
The closest measured variant disabled outbound receipt RPCs but retained
receiver-side bookkeeping. This PR removes that bookkeeping too, so these are
historical supporting measurements, not a benchmark of this exact PR commit.
...
do not isolate an effect size for each of the five changes.
