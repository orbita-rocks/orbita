...
| Image tags     | `develop`, `0.2.0-dev`, `sha-abc1234` | `0.1.0`, `0.1`, `latest` |
...
expire after 30 days; release tags and the moving `develop` and `-dev` tags are
never selected by that cleanup, because it only deletes an image whose tags are
all `sha-` prefixed.

The `-dev` version is published as an image tag as well as the moving `develop`
one, because `develop`'s own install paths pin it. `bump-version.sh` writes the
workspace version into the image tags in `deploy/manifests/orbita.yaml` and
into the chart's `appVersion`, which `values.yaml` defaults the image tag to.
If nothing published `0.2.0-dev`, applying the manifests off `develop` would
fail to pull. It floats the same way `develop` does, so a bug report should
still quote the `sha-` tag.
