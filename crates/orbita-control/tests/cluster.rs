...
/// Replays the log and, after every single entry, requires that every probe
/// key resolves to exactly one owned partition and that no split child is ever
/// in the map at the same time as its parent.
///
/// This is the continuous read the acceptance criterion asks for, done against
/// the committed log rather than a live client: the map only changes at commit
/// points, so if the property holds after every committed entry there is no
/// instant between two entries at which it did not. The exactly-one part is
/// two facts together — `check_coverage` rules out a key owned by two
/// partitions, and the owner check rules out a key owned by none — and the
/// child/parent overlap check is the one that would fail against the old
/// one-entry split's window, where a child was named an owner before it had
...
fn every_key_has_one_owner_through_the_split(
    ...
    for (position, command) in entries.iter().enumerate() {
        ...
        // Nothing to probe until the keyspace exists, which is the first entry
        // that puts the parent in the map. Before that a lookup is honestly
        // empty rather than a hole.
        let keyspace_live =
        ...
        for key in probes {
            match map.lookup(keyspace, key) {
                ...
                Some(info) => {
                    return Err(format!(
                        "key {key:?} resolved to partition {} with no owner after entry \
                         {position} ({command:?})",
                        info.id
                    ))
                }
                None => {
                    return Err(format!(
                        "key {key:?} resolved to no partition at all after entry {position} \
                         ({command:?})"
                    ))
                }
            }
...
#[test]
fn a_manual_split_leaves_every_key_owned_by_exactly_one_partition_through_the_split() {
    // The headline for issue #97 and the acceptance criterion at
    // docs/plan/03-control.md:91. Reproduce a failure with:
    //   ORBITA_SIM_SEED=<seed> cargo test -p orbita-control --test cluster \
    //     a_manual_split_leaves_every_key_owned_by_exactly_one_partition_through_the_split
    check_seeds(
        "a_manual_split_leaves_every_key_owned_by_exactly_one_partition_through_the_split",
        16,
        |seed| {
            ...
            // ADR 0002: the children continue the parent's Lamport sequence and
            // sit one epoch above it, so no key's version regresses and an
            // in-flight parent write is fenced rather than misfiled.
            for child in [lower, upper] {
                let epoch = map.partition(child).expect("a live child").epoch;
                if epoch <= parent_epoch {
                    return Err(cluster.sim.failure(format!(
                        "child {child} is at epoch {epoch:?}, not above the parent's \
                         {parent_epoch:?}"
                    )));
                }
            }
